...
import type { AntigravityConfig } from './AntigravityCanvas';

interface ControlsProps {
    config: AntigravityConfig;
    onConfigChange: (newConfig: AntigravityConfig) => void;
}

export default function Controls({ config, onConfigChange }: ControlsProps) {
    const [isOpen, setIsOpen] = useState(true);

    const handleChange = (key: keyof AntigravityConfig, value: number | string) => {
        onConfigChange({
            ...config,
            [key]: value,
        });
    ...
    const handleSave = () => {
        ...
        downloadAnchorNode.setAttribute('download', 'antigravity-config.json');
    ...
    return (
        <div className="fixed top-4 right-4 w-80 bg-black/80 backdrop-blur-lg border border-white/10 rounded-xl p-6 text-white z-50 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-lg font-medium tracking-tight">Animation Settings</h2>
                <button
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between text-sm text-white/70">
                        ...
                        <span>{config.flowIntensity.toFixed(1)}</span>
                    </div>
                    <input
                        type="range"
                        min="0"
